...
func CreatePodFromTeplate(ctx context.Context, clientSet *kubernetes.Clientset,
	...
	// 创建 Pod
	newPod, err := clientSet.CoreV1().Pods(pod.Namespace).Create(ctx, pod, metav1.CreateOptions{})
...
func WaitPodComplete(ctx context.Context, clientSet *kubernetes.Clientset, hPod *v1.Pod) error {
	defer func() {
...
func GenePvFromTeplate(ctx context.Context, podTemplate string, podStruct interface{}) (*v1.PersistentVolume, error) {
	initPvBytes, err := ParseTemplate(podTemplate, podStruct)
	if err != nil {
		klog.Errorf("ParseTemplate error: %v", err)
		return nil, err
	}
