...
const StateConfigmapName = "simplecsi"
const StateVolumeKey = "volume"
...
type State interface {
	...
	SetKubernetesClient(clientset *kubernetes.Clientset)
}
...
// are not saved.
func New(statefilePath string, clientset *kubernetes.Clientset) (State, error) {
	s := &state{
...
func (s *state) dump() error {
	...
	err = k8sclient.CreateOrUpdateConfigmap(context.Background(), s.clientset, StateConfigmapName, "default",
		StateVolumeKey, string(data))
	if err != nil {
		return err
	}
	return nil
...
func (s *state) restore() error {
	...
	dataStr, err := k8sclient.GetConfigmapData(context.Background(), s.clientset, StateConfigmapName, "default",
		StateVolumeKey)
	if err != nil {
		klog.Errorf("get configmap error: %v", err)
		return err
	}
	if len(dataStr) > 0 {
		if err := json.Unmarshal([]byte(dataStr), &s.resources); err != nil {
...
func (s *state) GetVolumeByID(volID string) (Volume, error) {
	err := s.restore()
	if err != nil {
		klog.Errorf("GetVolumeByID %v", err)
		return Volume{}, err
	}
...
func (s *state) GetVolumeByName(volName string) (Volume, error) {
	err := s.restore()
	if err != nil {
		klog.Errorf("GetVolumeByName %v", err)
		return Volume{}, err
	}
	for _, volume := range s.Volumes {
...
func (s *state) UpdateVolume(update Volume) error {
	err := s.restore()
	if err != nil {
		klog.Warningf("UpdateVolume %v", err)
	}
...
func (s *state) DeleteVolume(volID string) error {
	err := s.restore()
	if err != nil {
		klog.Errorf("DeleteVolume %v", err)
		return err
	}
	for i, volume := range s.Volumes {
		if volume.VolID == volID {
...
func (s *state) GetKubernetesClient() *kubernetes.Clientset {
	return s.clientset
}
func (s *state) SetKubernetesClient(clientset *kubernetes.Clientset) {
